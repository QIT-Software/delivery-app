...
export default abstract class IOrderManager {
    ...
    abstract createOrder(cartId: ID, userId: ID, setId: ID, clientAddress: {
        lat: number;
        lng: number;
        description: string;
    }, restaurantId: ID, numberOfDays: number): Promise<Order>;
    abstract setOrderRestaurant(id: ID, restaurantId: ID, orderInfoId: string, priceCents: number): Promise<void>;
    ...
    abstract evaluateOrder(role: AppType, userId: string, orderId: string, rating: number): Promise<void>;
    abstract acceptOrder(userId: string, orderId: string): Promise<void>;
    abstract markOrder(appType: AppType, orderId: string, bagId: string): Promise<void>;
