import React from 'react';
import styles from 'courier/src/routes/main/documentPicker/DocumentPickerPopUp.styles';
import {useDocumentsActions} from 'state/courier/hooks/UseActions';
import {useRouterActions} from 'state/hooks/UseActions';
import showImagePicker from 'utils/ImagePickerUtil';
import {
  Modal,
  Text,
  TouchableOpacity,
  TouchableWithoutFeedback,
  View,
} from 'react-native';

const DocumentPickerPopUp: React.FC = () => {
  const actions = useDocumentsActions();
  const routerActions = useRouterActions();
  const avatarPress = async (options: 'camera' | 'gallery') => {
    const result = await showImagePicker(options);
    if (!result.cancelled) {
      await actions.addDocument('employmentAgreement', result.uri);
      await routerActions.goBack();
    }
  };
  return (
    <Modal animationType="fade" transparent>
      <TouchableWithoutFeedback onPress={() => routerActions.goBack()}>
        <View style={styles.modalOuter}>
          <View style={styles.container}>
            <TouchableOpacity style={styles.button} onPress={() => avatarPress('camera')}>
              <Text style={styles.buttonText}>Camera</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => avatarPress('gallery')}>
              <Text style={styles.buttonText}>Gallery</Text>
            </TouchableOpacity>
          </View>
          {/* <View style={styles.cancelButton}> */}
          {/*  <TouchableOpacity onPress={() => routerActions.goBack()}> */}
          {/*    <Text style={styles.cancelText}>Cancel</Text> */}
          {/*  </TouchableOpacity> */}
          {/* </View> */}
        </View>
      </TouchableWithoutFeedback>
    </Modal>
  );
};

export default DocumentPickerPopUp;
